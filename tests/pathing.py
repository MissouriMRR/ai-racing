...
class ReproduceResetRaceCondition:
    ...
    def give_control_stick_inputs(self, roll, pitch, yaw, z, duration):
        ...
        """
        self.airsim_client.moveByRollPitchYawZAsync(roll, pitch, yaw, z, duration, vehicle_name=self.drone_name)
...
def get_drone_y_velocity(reproducer, current_position):
    ...
    y_velocity = ((new_position.position.y_val - current_position.position.y_val) / .01)
    x_velocity = -((new_position.position.x_val - current_position.position.x_val) / .01)
    drone_orientation = airsimdroneracinglab.utils.to_eularian_angles(new_position.orientation)
    yaw_angle = drone_orientation[2]
    drone_y_velocity  = (x_velocity / math.cos(yaw_angle)) + (y_velocity / math.cos(yaw_angle - (math.pi / 2)))
    if  0 > yaw_angle > -(math.pi / 2):
        drone_y_velocity = -drone_y_velocity
    elif math.pi > yaw_angle > (math.pi / 2):
        drone_y_velocity = -drone_y_velocity
    if not(.25 > drone_y_velocity > -.25):
        if drone_y_velocity > 0:
            drone_y_velocity = ((abs(drone_y_velocity) - 1) / (4 * abs(drone_y_velocity))) + .75
        else: 
            drone_y_velocity = -(((abs(drone_y_velocity) - 1) / (4 * abs(drone_y_velocity))) + .75)
...
if __name__ == "__main__":
    ...
    reproducer.takeoff()
    reproducer.give_control_stick_inputs(0, .3, drone_orientation[2]+math.pi, drone_pose.position.z_val, 2)
    time.sleep(3)
    ...
    for i in range(25):
        ...
        while not location_reached:
            ...
            if y_velocity > .15:
                roll_angle = -.1 * y_velocity
            elif y_velocity < -.15:
                roll_angle = -.1 * y_velocity
            else:
                roll_angle = 0
            print('Roll angle:', roll_angle)
            reproducer.give_control_stick_inputs(roll_angle, .02, yaw_angle, gate_pose.position.z_val, duration)
            distance_to_gate = get_distance_to_gate(vector_to_gate, i)
            print()

            if distance_to_gate < 1.5:
                # time.sleep(10)
