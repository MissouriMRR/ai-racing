    ...
    def get_drone_velocity_components(
        ...
        ----------
            ...
            z_velocity_deadzone : float
                Values between this value +- returned as 0 (defaults to .05)s
...
if __name__ == "__main__":
    ...
    pitch_PID: PID = PID(kp=0.5, max_output=0.05)
    pitch_PID.set_target(3)
    roll_PID: PID = PID(kp=0.1, max_output=0.2)
    ...
    for next_gate in range(NUM_GATES):
        ...
        while distance_to_gate > 1:
            ...
            drone_velocity_components: tuple[float, float, float] = (
                reproducer.get_drone_velocity_components(
                    -math.degrees(drone_orientation[2])
                )
            )
            ...
            drone_z_velocity_input: float = -throttle_PID.adjust_output(
                drone_pose.position.z_val
            )
            z_velocity_PID.set_target(drone_z_velocity_input)
            drone_throttle_input: float = 0.5938 + z_velocity_PID.adjust_output(
                drone_z_velocity
            )
            drone_roll_angle = roll_PID.adjust_output(drone_y_velocity)
            print("Roll: ", drone_roll_angle)
